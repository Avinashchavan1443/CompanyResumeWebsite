...
$file=fopen('reg1.txt', 'r');
while (!feof($file)) {
	$arr=fgets($file);
	$arr2=explode('|', $arr);
	if($userN==$arr2[0] && $passW==$arr2[1])
	{
		$success=true;
		fclose($file);
		break;
...
$userlist = file_get_contents('reg2.txt');
$i=0;
$success=false;
$user=explode('|', $userlist);
while (!empty($user[$i])) {
	if ( $userN == $user[$i] && $passW ==  $user[$i+1]) {
        $success = true;
        break;
	}
	$i=$i+2;
...
if ($success) {
   // echo "<br> Hi $userN you have been logged in. </br>";
	...
	<body>
		<div></div>
		<h3>You have entered the wrong username or password. Please try again <a href='jobs.html'>here.</a></h3>
	</body>
	</html>";
	exit();
...
<?php 
		...
		fwrite($file, $seconds);
      	...
      	//add($abc);
      	//setcookie("time",$seconds,time()+360000);
      	//echo $seconds;
      ?>
